...
import { X, Trash2, Plus, Minus, Tag, Check, ArrowRight, ShoppingBag, Sparkles, User, Phone, MapPin, MessageSquareText, AlertCircle } from 'lucide-react';
...
export default function CartDrawer() {
  ...
  const handleWhatsappChange = (val) => {
    ...
    if (invalidField === 'whatsappNumber' && val.trim()) {
      setFormError('');
      setInvalidField('');
    }
  };
  ...
  const handleProcessCheckout = () => {
    ...
    if (!whatsappNumber.trim()) {
      setFormError('Mohon isi Nomor WhatsApp (untuk mengirim invoice)!');
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
            ...
            {cart.length === 0 ? (
              <div className="py-20 text-center space-y-3">
                ...
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="mt-2 inline-flex items-center gap-1 bg-slate-900 text-white font-extrabold text-xs px-4 py-2.5 rounded-xl shadow-xs"
                >
                  Pilih Menu Sekarang
                </button>
              </div>
              ...
              <>
                  ...
                  {cart.map((cartEntry) => (
                    ...
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <div className="flex flex-wrap gap-1 text-[10px] text-slate-500">
                            {cartEntry.selectedBean && (
                              <span className="bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 font-bold text-slate-800">
                                {cartEntry.selectedBean.name}
                              </span>
                            )}
                            ...
                            {cartEntry.selectedToppings?.map((t) => (
                              <span key={t.id} className="bg-slate-100 text-slate-800 px-1.5 py-0.5 rounded border border-slate-300">
                                +{t.name}
                              </span>
                            ))}
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-1">
                            ...
                            <div className="flex items-center bg-slate-900 text-white rounded-lg p-0.5 font-black">
                              <button
                      ...
                      <div className="pt-2 border-t border-slate-100 flex items-center gap-2">
                        <MessageSquareText className="w-3.5 h-3.5 text-slate-700 shrink-0" />
                        <input
                          type="text"
                          ...
                          className="w-full bg-slate-50 border border-slate-200 focus:border-slate-700 text-[11px] text-slate-800 placeholder-slate-400 rounded-lg px-2.5 py-1 outline-none"
                        />
                      </div>
                    </div>
                  ))}
                ...
                {crossSellItems.length > 0 && (
                  <div className="bg-slate-100 border border-slate-200 p-3 rounded-2xl space-y-2.5">
                      ...
                      {crossSellItems.map((cItem) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="mt-1.5 w-full border border-slate-800 text-slate-800 hover:bg-slate-800 hover:text-white font-black text-[10px] py-1 rounded-lg flex items-center justify-center gap-0.5 transition-colors active:scale-95"
                          >
                            <Plus className="w-3 h-3 stroke-[3]" />
                            <span>Tambah (+)</span>
                          </button>
                ...
                <div className={`p-3.5 rounded-2xl space-y-3 transition-colors ${
                  invalidField ? 'bg-rose-50/70 border-2 border-rose-400' : 'bg-slate-100 border border-slate-200'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1">
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] font-bold text-slate-800 flex items-center gap-1">
                        <User className="w-3.5 h-3.5 text-slate-700" /> Nama Pemesan
                      </label>
                      <input
                        ref={nameInputRef}
                        type="text"
                        ...
                        className={`w-full bg-white text-xs text-slate-900 placeholder-slate-400 placeholder:italic rounded-xl px-3 py-2.5 outline-none font-bold transition-all ${
                          invalidField === 'customerName'
                            ...
                            : 'border border-slate-300 focus:border-slate-800'
                        }`}
                      />
                      {invalidField === 'customerName' && (
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] font-bold text-slate-800 flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5 text-slate-700" /> Nomor Meja (Lihat stiker di meja kamu)
                      </label>
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ref={tableInputRef}
                          ...
                          className={`bg-white font-black text-sm rounded-xl px-3 py-2 outline-none text-center shadow-xs placeholder-slate-400 placeholder:italic placeholder:font-normal transition-all ${
                            invalidField === 'tableNumber'
                              ...
                              : 'border-2 border-slate-800 text-slate-900'
                          }`}
                        />
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] font-bold text-slate-800 flex items-center gap-1">
                        <Phone className="w-3.5 h-3.5 text-slate-700" /> Nomor WhatsApp (Wajib untuk mengirim invoice)
                      </label>
                      <input
                      ...
                      {invalidField === 'whatsappNumber' && (
                        <p className="text-[10px] font-bold text-rose-600 flex items-center gap-1 pt-0.5">
                          <AlertCircle className="w-3 h-3" /> * Nomor WhatsApp wajib diisi (untuk mengirim invoice)
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-slate-50 border border-slate-200 p-3 rounded-2xl space-y-2">
                  <div className="flex items-center justify-between text-xs font-bold text-slate-700">
                    ...
                    <span className="text-[10px] text-slate-500">Coba: TILEHAUSE20</span>
                  </div>

                  {appliedVoucher ? (
                    <div className="flex items-center justify-between bg-slate-100 border border-slate-300 p-2 rounded-xl text-xs">
                      <div className="flex items-center gap-2">
                        <Check className="w-4 h-4 text-slate-800" />
                        <div>
                          <div className="font-black text-slate-900">{appliedVoucher.code}</div>
                    ...
                    <form onSubmit={handleApplyVoucher} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-white border border-slate-200 focus:border-slate-800 text-xs text-slate-800 uppercase placeholder-slate-400 rounded-xl px-3 py-2 outline-none"
                      />
                      <button
                        type="submit"
                ...
                <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-2xl space-y-2 text-xs">
                  ...
                  <div className="pt-2 border-t border-slate-200 flex justify-between text-sm font-black text-slate-900">
                    ...
                    <span className="text-slate-900 text-base">{formatIDR(totalPrice)}</span>
                  </div>
                </div>
              </>
            )}
          ...
          {cart.length > 0 && (
            <div className="p-4 bg-slate-50 border-t border-slate-200 shrink-0">
              <button
                ...
                className="w-full bg-slate-900 text-white font-black text-sm py-3.5 rounded-2xl flex items-center justify-between px-4 shadow-md active:scale-95 transition-all hover:bg-slate-800"
              >
