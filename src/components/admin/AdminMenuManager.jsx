...
export default function AdminMenuManager() {
  const { menuItems, categories, initRealtimeMenu, isLoading } = useMenuStore();
  ...
  const handleOpenEditModal = (item) => {
    ...
    setFormCategoryId(item.category || item.category_id || '');
    setFormPrice(item.price ? item.price.toString() : '');
    setFormDescription(item.description || '');
    setFormImageUrl(item.image || item.image_url || '');
    
    // Tentukan status sold out berdasarkan is_available
    const isCurrentlySoldOut = item.is_available !== undefined 
      ? !item.is_available 
      : (item.isSoldOut || item.is_sold_out || false);
      
    setFormIsSoldOut(isCurrentlySoldOut);
    setIsModalOpen(true);
  };
  ...
  const handleSaveMenu = async (e) => {
    ...
    // Payload menyimpan is_available sebagai kebalikan dari formIsSoldOut
    const payload = {
      name: formName,
  ...
  const handleToggleSoldOut = async (item) => {
    const currentIsAvailable = item.is_available !== undefined 
      ? item.is_available 
      : !(item.isSoldOut || item.is_sold_out);
    ...
    try {
      ...
      alert('Gagal memperbarui status menu: ' + err.message);
    }
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-zinc-800/80 text-xs">
              {isLoading ? (
                <tr><td colSpan="5" className="py-8 text-center text-zinc-500">Memuat data...</td></tr>
              ) : filteredMenu.length > 0 ? (
                filteredMenu.map(item => {
                  const isSoldOut = item.is_available !== undefined 
                    ? !item.is_available 
                    : (item.isSoldOut || item.is_sold_out);
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-zinc-800/40 transition-colors">
                      ...
                      <td className="py-3 px-4 text-zinc-400">{getCategoryName(item)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSaveMenu} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-[11px] font-mono text-zinc-400 block mb-1">Deskripsi (Opsional)</label>
                <textarea value={formDescription} onChange={e => setFormDescription(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-white text-sm outline-none" rows={2} />
              </div>
              <div>
                <label className="text-[11px] font-mono text-zinc-400 block mb-1">URL Gambar (Opsional)</label>
                <input type="text" value={formImageUrl} onChange={e => setFormImageUrl(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-white text-sm outline-none" placeholder="https://..." />
              </div>

              {/* Input Switch Status Stok / Sold Out di Form Modal */}
              <div className="flex items-center justify-between p-3.5 bg-zinc-950 border border-zinc-800 rounded-xl">
                <div>
                  <label className="text-xs font-bold text-white block">Status Stok Menu</label>
                  <span className="text-[10px] text-zinc-400">Aktifkan untuk menandai menu sebagai Sold Out</span>
                </div>
                <button
                  type="button"
                ...
                >
                  {formIsSoldOut ? (
                    <><Ban className="w-3.5 h-3.5" /> Sold Out (Habis)</>
                  ) : (
